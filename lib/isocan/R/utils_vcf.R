...
read_MANOmutect <- function(path, sample_name) {
	# supports point mutation only for now
	readr::read_tsv(path, comment = "##") %>% 
		rename(chr = contig, end = position, ref = ref_allele, alt = alt_allele) %>%
		mutate(start = end - 1,
			type = "*",
			sample = sample_name,
			somatic = NA,
...
read_MANOsid <- function(path, sample_name) {
	readr::read_tsv(path, comment = "##") %>%
		rename(
			...
			somatic = INFO) %>%
		mutate(
			ref = str_sub(ref, start = 2L) %>% str_to_lower(),
			alt = str_sub(alt, start = 2L) %>% str_to_lower(),
			end = start + str_length(ref),
			somatic = (somatic == "SOMATIC"),
			type = sapply(str_length(alt) - str_length(ref), function(x)
				if (x > 0) {"+"}
				else if (x < 0) {"-"}
				else {stop("Invalid argument: non-indel variants detected.")}),
			sample = sample_name)
}
...
read_genomon_variant <- function(path) {
	readr::read_tsv(path, 
		col_types = readr::cols_only(Chr = "c", Start = "i", End = "i", Ref = "c", Alt = "c"),
		comment = "#") %>%
		`colnames<-`(c("chr", "start", "end", "ref", "alt")) %>%
		mutate(
			start = if_else(ref == "-", start, start - 1L),
			#End = End,
