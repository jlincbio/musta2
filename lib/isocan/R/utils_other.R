...
read_gtf <- function(file) {
	...
	gtf <- as_tibble(rtracklayer::import(con = file, format = "gtf"))
	gtf <- gtf %>% mutate_if(is.factor, as.character) %>% select(-width) %>% rename(feature = "type", frame = "phase", seqname = "seqnames")
	# below added for compatibility with older version of tidyr
	gtf$score <- ifelse(is.na(gtf$score), yes = ".", no = as.character(gtf$score)) 
	gtf$frame <- ifelse(is.na(gtf$frame), yes = ".", no = as.character(gtf$frame))
	# above added for compatibility with older version of tidyr
