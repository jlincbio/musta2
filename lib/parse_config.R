...
####software####
softwares <- 
  c("sqanti_filter.py", "sqanti_qc.py", "minimap2", "samtools", "salmon", "seqkit")
software_path_argnames <- 
  c("sqanti_filter", "sqanti_qc", "minimap2", "samtools", "salmon", "seqkit")
soft_ver_comm <- 
...
if (!args$no_salmon) {
	mandatory_cols <- c(mandatory_cols, "short_read_1", "short_read_2")
}

if (args$use_lq) {
	mandatory_cols <- c(mandatory_cols, "long_read_lq")
}
...
## below added 01/12/22 for testing whether short-read can be safely omitted
walk2(c("long_read_hq", "cluster_report", "SJ"), c(FALSE, TRUE, TRUE), check_no_na)
if (!args$no_salmon) {
	check_no_na("short_read_1")
	check_no_na("short_read_2")
}
## above added 01/12/22 for testing whether short-read can be safely omitted
