...
## modified 01/12/22 to set lmem to h=mk01
## modified 05/30/22 to set default memory request to 16G
## modified 06/08/22 for resource limit modifications
...
mem <- function(x) {
  ...
  # edit: 20211029 - mem_req is not consumable on rg20 
  paste0("s_vmem=", x, "G") 
}
...
## @usage parallel_option(env = c("def_slot", "mpi", "mpi-fillup", "mpi_4", "mpi_8", "mpi_16", "mpi_24"), slot = 1L, memory = 16, master_memory = NULL, ljob = FALSE, no_rerun = TRUE, special_que = c(NULL, "cp", "docker", "knl", "gpu", "groupname"), docker_images = NA_character_)
...
#' @export
#parallel_option <- function(env = c("smp"def_slot", "mpi", "mpi-fillup", "mpi_4", "mpi_8", "mpi_16", "mpi_24"),
parallel_option <- function(env = c("smp"),
                            slot = 1L, memory = 16, master_memory = NULL,
  ...
  resource_df <- 
    resource_df %>%
    dplyr::mutate(SLOT_NODE = c(max_slot))
#    dplyr::mutate(SLOT_NODE = c(max_slot, max_slot, max_slot, 4L, 8L, 16L, 24L))
  ...
  if (!is.null(special_que)) {
    ...
    if (slot_node > 39L) {
        rlang::abort("Resource Error: number of slots must be equal to or less than 24 per node.", "requirement_resource_error")
    }
    if ((slot_node > 24L) | (total_memory > 128L)) {
	rlang::inform("Resource Warning: >24 total threads or >128GB of memory may be required.", "requirement_resource_warning")
	ljob <- FALSE
	lmem <- TRUE
    }
    if (total_memory > 500L) {
        rlang::abort("Resource error: total memory must be less than 500GB", "requirement_resource_error")
    }
    result <- 
      stringr::str_c(
        resource("-pe", env, stringr::str_c(slot, collapse = "-")),
        resource("-l", mem(memory)),
        ifelse(!is.na(ljob) && ljob, resource("-l", "h=mk01"), "") %>% character_1_0(),
        ifelse(!is.na(no_rerun) && no_rerun, resource("-q", "r.q"), "") %>% character_1_0(),
        ifelse(!is.na(lmem) && lmem, resource("-l", "h=mk01"), "") %>% character_1_0(),
