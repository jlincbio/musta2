...
add_pbcount <- function(df, cluster_report_path, id_colname = "seq_id") {
	warning(sprintf("Cluster report path: %s", cluster_report_path))
  extract_count <- function(df, id_colname) {
    ### ISOSEQ2 HARDCODING BELOW
    count_matrix <-
      df[[id_colname]] %>% 
      str_replace("^.+/f([:digit:]+)p([:digit:]+).+$", "\\1,\\2") %>% 
      str_split_fixed(",", 2)
    ### ISOSEQ2 HARDCODING ABOVE


    FLvec = as.integer(count_matrix[, 1])
    nFLvec = as.integer(count_matrix[, 2])
    if (any(is.na(FLvec)) || any(is.na(nFLvec))) {
		warning(sprintf("Count matrix dimension: %s", paste(as.character(dim(count_matrix)), collapse = " x ")))
		warning(sprintf("FL counts appear to be length %d: %s\n", length(FLvec), paste(as.character(FLvec), collapse = " ")))
		warning(sprintf("nFL counts appear to be length %d: %s\n", length(nFLvec), paste(as.character(nFLvec), collapse = " ")))
		stop("Failed to extract FL, nFL information from cluster_ids (this method cannot apply to Isoseq3). Please specify valid cruster_report.csv")
	}
    df$FL <- FLvec
    df$nFL <- nFLvec
    df
  ...
  if (is.null(cluster_report_path) || is.na(cluster_report_path)) {
    warning("Printing df for debugging...")
    warning(sprintf("Column names (in order): %s", paste(colnames(df), collapse = ":")))
    warning(head(df))
    warning("df$id_colnames:")
    warning(head(df$seq_id))
    #save(df, file = "/dev/shm/df_test-20220113.RData")
    extract_count(df, id_colname)
     ...
     warning(sprintf("Length of count_df$FL: %d (NA: %d)", length(count_df$FL), length(which(is.na(count_df$FL)))))
    # save(df, file = "/dev/shm/intermerge-20211112.RData")
	df_count_index_match <- rep(NA, length = dim(df)[1])
	for (ix1 in 1:length(count_df$cluster_id)) {
		cur_ix_df_match <- which(endsWith(df[[id_colname]], count_df$cluster_id[ix1]))
		if (length(cur_ix_df_match) == 1) {
			df_count_index_match[cur_ix_df_match[1]] <- ix1
		}
	}
	res <- cbind(df, count_df$FL[df_count_index_match], count_df$nFL[df_count_index_match])
	colnames(res) <- c(colnames(df), "FL", "nFL")
	res <- as_tibble(res)

    #res <- left_join(df, count_df, by = `names<-`("cluster_id", id_colname))
	warning(sprintf("Length of res$FL: %d (NA: %d)", length(res$FL), length(which(is.na(res$FL)))))
    if (any(is.na(res$FL))) {
