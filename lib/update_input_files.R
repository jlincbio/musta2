...
# originally modified 2022/04/08 for lordec-skipping
# modified 2022/06/08 to disable short-read data processing
if (args$no_salmon == FALSE) {
	input_files <- input_files %>% 
		mutate(interleave = path(sample_dir, "interleave.fastq"),
		interleave_lordectmp = paste0(interleave, "_k19_s3.h5"),
		lordec_hq = long_copy_hq, lordec_lq = long_copy_lq)
} else {
input_files <- input_files %>% mutate(lordec_hq = long_copy_hq, lordec_lq = long_copy_lq) # TODO fastq2fasta
}
